...
class jeeb extends base
{
  ...
  function after_process()
  {
    ...
    $baseUri = $this->testMode ? "http://test.jeeb.io:9876/" : "https://jeeb.io/" ;
    $signature = $this->api_key;

    $btc = \Jeeb\Merchant\Order::convertIrrToBtc($baseUri, $order->info['total'], $signature);
    $this->log('Signature ='.$signature.' base Url = '.$baseUri.' Convertion = '.$btc, true);
    ...
    $token = \Jeeb\Merchant\Order::createInvoice($baseUri, $btc, $params, $signature);
  ...
  function install()
  {
    ...
    $db->Execute("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, set_function, date_added) values ('Enable test mode?', 'MODULE_PAYMENT_JEEB_TEST', 'False', 'Enable test mode to test on sandbox.jeeb.com. Please note, that for test mode you must generate separate API credentials on sandbox.jeeb.com. API credentials generated on coingame.com will not work for test mode.', '6', '1', 'zen_cfg_select_option(array(\'True\', \'False\'), ', now())");
